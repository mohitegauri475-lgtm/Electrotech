...
public class DatabaseConfig {
    ...
    public DataSource dataSource(DataSourceProperties properties, Environment environment) {
        ...
        boolean isPostgresActive = Arrays.asList(environment.getActiveProfiles()).contains("postgres");

        if ((databaseUrl != null && !databaseUrl.trim().isEmpty()) || isPostgresActive) {
            if (databaseUrl != null && !databaseUrl.trim().isEmpty()) {
                try {
                    ...
                    // Connection pool settings for Render free/starter instances
                ...
                logger.info("PostgreSQL profile active without DATABASE_URL. Using configured PostgreSQL datasource properties.");
        ...
        logger.info("Local environment detected: using standard local datasource configuration.");
